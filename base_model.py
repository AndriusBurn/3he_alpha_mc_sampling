...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 15:23:08 EDT 2024
...
class BaseModel:
    ...
    def __init__(self, data : npt.ArrayLike, param_prior : npt.ArrayLike, norm_prior : npt.ArrayLike, use_theory_cov : bool):
        ...
        Parameters:
        - data (array-like): The input data containing the experimental measurements.
        - param_prior (array-like): The prior information for the model parameters.
        - norm_prior (array-like): The prior information for the normalization factors.
        - use_theory_cov (bool): Flag indicating whether to use the theoretical covariance matrix.

        Returns:
        None
        """
        # Unpack the data
        self.E_lab_cs = data[:, 0]
        ...
        self.kvalue_cs = self.k(self.E_lab_cs)
        self.kcvalue_cs = self.kc(self.E_lab_cs)
        self.etavalue_cs = self.eta(self.E_lab_cs)
        self.Hvalue_cs_real = np.real(self.H(self.E_lab_cs))
        self.Hvalue_cs_imag = np.imag(self.H(self.E_lab_cs))
        self.Hvalue_cs = self.Hvalue_cs_real + 1.j * self.Hvalue_cs_imag
        self.C0_2value_cs = self.C0_2(self.E_lab_cs)
        self.C1_2value_cs = self.C1_2(self.E_lab_cs)
    ...
    def cot(self, theta_deg : npt.ArrayLike) -> npt.ArrayLike:
        ...
        return 1 / np.tan(theta_deg * (np.pi / 180))
    
    def E_cm(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Convert energy from lab frame to CM frame
        return ((2 * self.mt) / (self.mt + self.mb +
                        self.sqrt(((self.mt + self.mb)**2) +
                        2 * self.mt * E_lab))) * E_lab
    
    def E_lab(self, E_cm : npt.ArrayLike) -> npt.ArrayLike:
        # Convert energy from CM from to lab frame
        return ((E_cm + (2 * (self.mt + self.mb))) /
                        (2.0 * self.mt)) * E_cm

    def kc(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain kc as a function of Elab
        return self.Zb * self.Zt * self.alpha * (
                        (self.mu + self.E_cm(E_lab)) / self.h_bar_c)
    
    def k(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain k as a function of Elab
        return (1 / self.h_bar_c) * self.sqrt(
                        ((self.mu + self.E_cm(E_lab))**2) - self.mu**2)
    
    def eta(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain eta as a function of Elab
        return self.kc(E_lab) / self.k(E_lab)
    
    def H(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain H as a function of Elab
        return ((psi((1.j) * self.eta(E_lab))) +
                    (1 / (2.j * self.eta(E_lab))) -
                    np.log(1.j * self.eta(E_lab)))

    def h(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Get the real part of H
        return self.H(E_lab).real

    def C0_2(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain C0^2 as a function of Elab
        return (2 * np.pi * self.eta(E_lab)) / (
                            np.exp(2 * np.pi * self.eta(E_lab)) - 1)

    def C1_2(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain coefficient C1_2 as a function of Elab
        return (1 / 9) * (1 + self.eta(E_lab)**2) * self.C0_2(E_lab)

    def C2_2(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain coefficient C2_2 as a function of Elab
        return (1 / 100) * (4 + self.eta(E_lab)**2) * self.C1_2(E_lab)
    
    def C3_2(self, E_lab : npt.ArrayLike) -> npt.ArrayLike:
        # Obtain coefficient C3_2 as a function of Elab
        return (1 / 441) * (9 + self.eta(E_lab)**2) * self.C2_2(E_lab)
